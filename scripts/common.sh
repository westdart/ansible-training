...
#######################################################################################################################
# Added for Openshift Automation

function getEnvSpecFile()
{
    [[ -z "${ENVSPEC}" ]] && { grep -Pzl "(?s)envs: \[.*\n.*prefix: '$(echo ${ENV_ID} | cut -d, -f1)'" ${ANSIBLE_DIR}/varfiles/* | tail -n1 ; return 0; }
    [[ -f "${ANSIBLE_DIR}/varfiles/${ENVSPEC}" ]]     && { echo "${ANSIBLE_DIR}/varfiles/${ENVSPEC}"; return 0; }
    [[ -f "${ANSIBLE_DIR}/varfiles/${ENVSPEC}.yml" ]] && { echo "${ANSIBLE_DIR}/varfiles/${ENVSPEC}.yml"; return 0; }
    [[ -f "${ENVSPEC}" ]] && { echo "$(cd $(dirname $ENVSPEC) && pwd)/$(basename $ENVSPEC)"; return 0; }
    return 1
}

function getEnvSpecTag()
{
    echo $(basename $(getEnvSpecFile)) | awk -F "." '{print $1}'
}

function getInventoryDir() {
    # The directory is relative to ${ANSIBLE_DIR}.
    echo "generated/$(getEnvSpecTag)/inventory"
}

function getAnsibleDebugClause() {
    [[ ${LOG_LEVEL} -ge ${LOG_TRACE} ]] && { echo '-vvv'; return 0; }
    [[ ${LOG_LEVEL} -ge ${LOG_DEBUG} ]] && { echo '-vv'; return 0; }
}
